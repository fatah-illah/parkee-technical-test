...
echo "Copying '$source_file' to $destination using SCP..."

# Use SCP to copy the file/directory
scp -r "$source_file" "$destination"

# Check if copy was successful
# shellcheck disable=SC2181
if [ $? -eq 0 ]; then
    echo "File/directory '$source_file' has been successfully copied to $destination"
else
    echo "Failed to copy file/directory to remote server."
    exit 1
fi
